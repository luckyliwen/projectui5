    ...
    */
	return Controller.extend("csr.lib.BaseController", {
		...
		addHtmlToForm: function( form, label, text ) {
			...
			var content = Util.fmtContentAsHtml(text);
	    ...
	    if (selKey) {
	    	for (var i=0; i < this.aSubProject.length; i++) {
	    		if (selKey == this.aSubProject[i].info) {
	    			...
	    			if (subPrj.startDate) {
	    				info += "  Start date: " + subPrj.startDate;
	    			}
	    			if (subPrj.startTime) {
	    				info += "  Start time: " + subPrj.startTime;
	    			}
	    			if (subPrj.endDate) {
	    				info += "  End date: " + subPrj.endDate;
	    			}
	    			if (subPrj.endTime) {
	    				info += "  End time: " + subPrj.endTime;
	    			}
	    			if (subPrj.location) {
	    				info += "  Location: " + subPrj.location;
	    			}
	    			if (subPrj.description) {
	    				info += "  Description: " + subPrj.description;	
	    			}
	    			oTextAreaInfo.setValue(info);
	    			oTextAreaInfo.setTooltip(info);
	    		}
	    	}
	    } else {
	    	this.subProjectIndex = -1;
	    	oTextAreaInfo.setValue("");
	    	oTextAreaInfo.setTooltip("");
	    }
	},
			...
			switch (cfg.type) {
				...
				case Enum.ControlType.List:
					...
					if (cfg.bSubProject) {
						...
						var textArea = new sap.m.TextArea({rows: 1, editable: false,
							placeholder: "Select above list to see detail"});
						form.addContent(textArea);
						select.attachChange( textArea, this.onSubProjectChanged, this);
					}
					break;
