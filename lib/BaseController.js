    ...
    */
	return Controller.extend("csr.lib.BaseController", {
		...
		createProjectHeader: function(projectCfg) {
			...
			//!!later considerate use the simple format if it is in same day
			if ( this.projectCfg.StartDate || this.projectCfg.StartTime ) {
				this.addTextToForm(form, "Start date & time ", 
					Util.fmtDateTime(this.projectCfg.StartDate, this.projectCfg.StartTime));
			}

			if ( this.projectCfg.EndDate || this.projectCfg.EndTime ) {
				this.addTextToForm(form, "End date & time ", 
					Util.fmtDateTime(this.projectCfg.EndDate, this.projectCfg.EndTime));
			}
