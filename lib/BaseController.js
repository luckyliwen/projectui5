sap.ui.define([
...
], function(Controller, Config, Enum,SimpleForm,Util,FileUploaderExt) {
	"use strict";
    ...
    */
	...
	createRegisterForm: function(aFormCfg) {
		...
		form.addContent( new sap.m.Label({text: "SAP ID and Name"}));
		form.addContent( new sap.m.Input({value: "{/UserId}", enabled: false}));
		form.addContent( new sap.m.Input({value: "{/SapUserName}", enabled: false}));
		...
		for (var i=0; i < aFormCfg.length; i++) {
			...
			switch (cfg.type) {
				...
				case Enum.ControlType.List:
					...
					Util.addItemsToSelect(select, cfg.candidate);
				...
				case Enum.ControlType.Attachment:
					...
					fileUploader.data('type', Util.getTypeFromLabel(cfg.label));
					if (this.aUploader)
