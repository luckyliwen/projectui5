...
var saveInSafari = function(data, fileName) {
	...
	var mUtil = {
		/**
		 * Get the Date instance from Date String like '9/16/16, 9:00 AM'
		 * As now in Chrome, it will handle it correct, but IE or Firefox will treat it as 1916
		 * @param  {[type]} sDate [description]
		 * @return {[type]}       [description]
		 */
		getDateFromString: function( sDate) {
		    // var regexp= new RegExp("(\\d+)/(\\d+)/(\\d+),(.*)$");
		    //now need support both format: old use '9/16/16, 9:00 AM', new format use:  1471221350000
		    var timeRegexp = /^\d+$/;
		    if (timeRegexp.test(sDate)) {
		    	return new Date( parseInt(sDate));
		    }

		    var regexp= new RegExp("(.*)/(\\d+)(.*)$");
		    var match = regexp.exec(sDate);
		    if (match) {
		    	var str = match[1] + "/20" + match[2] + match[3];
		    	return new Date(str);
		    } else {
...
// statusText: "Internal Server Error"
		...
		//now if call saveToFile(), some Chinese can't display correct. Later can directly  use ui5/core/util/File
		saveToCsvUtf8: function(content, fileName) {
		...
		exportTableContent: function( oTable, fileName , filterProjectId) {
			...
			oExport.generate().then(function(sContent) {
				...
				that.saveToCsvUtf8(newContent, fileName);
			});
		},

		/**
		 * To format the content as simple html format, now support following:
		 *  a:  http://,  https://  mailto:  
		 *  the \n will replace as <br>
		 * @param  {[type]} text 
		 * @return {[type]} tag:  normally use div/tag,  but can omit also
		 */
		fmtContentAsHtml : function(text, tag) {
			text = text.trim();
			//it already is html fomat
		    ...
		    for (var i=0; i < aLine.length; i++) {
		    	if (i != 0) 
		   		...
		   		var regexp = new RegExp( "(http://|https://|mailto:[ ]*)([^ ]+)")
		    	var  line = aLine[i];
		   		while (true) {
			   		var match = regexp.exec(line);  	
			    	if (match) {
			    		var whole = match[0];
			    		var protocol = match[1].trim();
			    		var href = match[2];
			    		
			    		//before the http:// part just copy 
			    		var pos = line.indexOf( whole );
			    		html += line.substr(0, pos);

			    		//http:// part convert to <a href= targe="_blank"></a>
			    		var link = "<a href='{0}' target='_blank'>{1}</a>";
			    		html += link.sapFormat(protocol + href, protocol + href)
			    		
			    		//after the http:// part continue convert 
			    		line = line.substr(pos + whole.length);
			    	} else {
			    		html += line;
			    		break;
			    	}
		...
		//!!later need use the long format
		...
		fmtDuration: function( start, end ) {
		    // return "From: " + start + " To: " + end;
		    return "From: " + mUtil.fmtTime(start) + " To: " + mUtil.fmtTime(end);
		    
		},
		

		//the sub-project date/time have high priority,it defined,then use it, otherwise, use the project level 
		fmtDateTimeSmart: function(projectDate, projectTime,  subDate, subTime ) {
			return mUtil.fmtDateTime( 
				subDate ? subDate : projectDate,
				subTime ? subTime : projectTime
				);
		},
		...
		fmtRowsByContent: function( text ) {
			if ( !text)
				return 1;
		    var aLine = text.split("\n");
		...
		isNumber: function(text, allowEmpty ) {
		    if (allowEmpty) {
		...
		isValidDuration: function(startDateTime, endDateTime) {
			/*var sStart = startDateTime,  sEnd = endDateTime;
			var timeRegexp = /^\d+$/;
			if ( timeRegexp.test(sStart)) {
				sStart = parseInt(sStart);
			}
			if ( timeRegexp.test(sEnd)) {
				sEnd = parseInt(sEnd);
			}

		    var start = new Date(sStart) , end = new Date(sEnd);*/

		    var start = mUtil.getDateFromString(startDateTime),  end = mUtil.getDateFromString(endDateTime);
		    var diff = end - start;
		...
		fmtTime: function( time ) {
			...
			if ( !  (time instanceof Date) ) {
				//!!previous here just call this.getDateFromString, it is wrong as the formatter will use control as the 'this'
				time = mUtil.getDateFromString(time);
			}
		...
		//like 2016-08-25T15:53:54"
		fmtTimeAsInput: function( timeValue ) {
			function addPrefix(v) {
				if ( v >= 10)
					return v;
				else
			...
			//considerate the timezone, need use UTC
			var time = new Date( timeValue.getTime() + timeValue.getTimezoneOffset() * 60 * 1000);

	    	var ret = "{0}-{1}-{2}T{3}:{4}:{5}"
	    	return ret.sapFormat( addPrefix( time.getYear() + 1900) ,
	    		addPrefix( time.getMonth() + 1),  addPrefix(time.getDate()),
	    		addPrefix(time.getHours()) , addPrefix(time.getMinutes()), addPrefix(time.getSeconds()) );
		},
