sap.ui.define([
	...
	function(MessageBox, MessageToast, Config,Export,ExportTypeCSV) {
	"use strict";
...
var saveInSafari = function(data, fileName) {
	...
	return {
		getTypeFromLabel: function( label ) {
		    return label.replace(/\s/g, '');
		},

		addItemsToSelect: function( select, value) {
			//first one always is the select mark
			var aValue = value.split(";");
			aValue.unshift("------Please Choose------");
		    for (var i=0; i < aValue.length; i++) {
		    	var item = aValue[i].trim();
		    	select.addItem( new sap.ui.core.Item({key: item, text: item}));
		    }
		},
		...
		createNewRegistration: function( oldRegister ) {
		    ...
		    return {
		    	...
		    	SapUserName: oldRegister.SapUserName,
...
// statusText: "Internal Server Error"
		...
		exportTableContent: function( oTable, fileName ) {
		    ...
		    var oExport = new Export({
			    ...
			    columns:  this._getExportColumns(oTable),
			});

			var that = this;
			oExport.generate().then(function(sContent) {
				that.saveToFile(sContent, fileName);
			});
		},
